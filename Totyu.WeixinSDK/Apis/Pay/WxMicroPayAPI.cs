...
namespace Totyu.WeixinSDK.Apis.Pay
{
    /// <summary>
    /// 微信支付 => 被扫支付 
    /// https://pay.weixin.qq.com/wiki/doc/api/jsapi.php?chapter=9_10
    /// </summary>
    public class WxMicroPayAPI : BaseAPI
    {
        /// <summary>
        /// 提交被扫支付API
        /// https://pay.weixin.qq.com/wiki/doc/api/jsapi.php?chapter=9_10
        ...
        /// <param name="out_trade_no">(必填) String(32) 商家订单ID,32个字符内、可包含字母, 其他说明见第4.2节商户订单号 https://pay.weixin.qq.com/wiki/doc/api/jsapi.php?chapter=9_10 </param>
        /// <param name="total_fee">(必填) Int 订单总金额，只能为整数，详见第4.2节支付金额:https://pay.weixin.qq.com/wiki/doc/api/jsapi.php?chapter=9_10 </param>
        /// <param name="fee_type">符合ISO 4217标准的三位字母代码，默认人民币：CNY，其他值列表详见第4.2节货币类型: https://pay.weixin.qq.com/wiki/doc/api/jsapi.php?chapter=9_10 </param>
        ...
        /// <returns>返回json字符串，格式参见：https://pay.weixin.qq.com/wiki/doc/api/jsapi.php?chapter=9_10 </returns>
        public static dynamic Submit(string appid, string mch_id, string device_info,
                                     ...
                                     string out_trade_no, int total_fee, string fee_type, string spbill_create_ip,
                                     string time_start, string time_expire, string goods_tag, string limit_pay,
                                     string auth_code,
                                     string partnerKey)
        {
            ...
            var _sign = WxPayAPI.Sign(stringADict, partnerKey);//生成签名字符串
            var _postdata = CommonHelper.GeneralPostdata(stringADict, _sign);
            var _url = ApiUrlHelper.MPApiUrl.MicroPay;
            return PostXmlAsync(_url, _postdata);
        }

        /// <summary>
        /// 撤销支付API
        /// https://pay.weixin.qq.com/wiki/doc/api/jsapi.php?chapter=9_11
        /// 应用场景：支付交易返回失败或支付系统超时，调用该接口撤销交易。如果此订单用户支付失败，
                                  ...
                                  string partnerKey)
        {
            ...
            var _sign = WxPayAPI.Sign(stringADict, partnerKey);//生成签名字符串
            var _postdata = CommonHelper.GeneralPostdata(stringADict, _sign);
            var _url = ApiUrlHelper.MPApiUrl.ReversePay;
            return PostXmlAsync(_url, _postdata);
        }
    }
}
