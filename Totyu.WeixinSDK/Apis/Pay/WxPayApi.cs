...
namespace Totyu.WeixinSDK.Apis.Pay
{
    /// <summary>
    /// 微信支付 => 公共API
    ...
    public class WxPayAPI : BaseAPI
    {
        ...
        /// <param name="limit_pay">(String(32) 指定支付方式 no_credit--指定不能使用信用卡支付)</param>
        ...
        /// <returns>返回json字符串，格式参见：https://pay.weixin.qq.com/wiki/doc/api/jsapi.php?chapter=9_1 </returns>
        public static dynamic UnifiedOrder(string appid, string mch_id, string device_info,
                                           ...
                                           string notify_url, string trade_type, string product_id, string limit_pay, string openid, string partnerKey
                                        )
        {
            ...
            var _sign = Sign(stringADict, partnerKey);//生成签名字符串
            var _postdata = CommonHelper.GeneralPostdata(stringADict, _sign);
            var _url = ApiUrlHelper.MPApiUrl.UnifiedOrder;
            return PostXmlAsync(_url, _postdata);
        }
        ...
        /// <param name="partnerKey">API密钥</param>
        /// <returns> 参见：https://pay.weixin.qq.com/wiki/doc/api/jsapi.php?chapter=9_2 </returns>
        /// <returns></returns>
        public static dynamic OrderQuery(string appid, string mch_id, string transaction_id, string out_trade_no, string nonce_str, string partnerKey)
        {
            ...
            var _sign = Sign(stringADict, partnerKey);//生成签名字符串
            var _postdata = CommonHelper.GeneralPostdata(stringADict, _sign);
            var _url = ApiUrlHelper.MPApiUrl.QueryOrder;
            return PostXmlAsync(_url, _postdata);
        ...
        /// <param name="nonce_str">随机字符串 随机字符串，不长于32位。</param>
        /// <param name="partnerKey">API密钥</param>
        /// <returns> 参见：https://pay.weixin.qq.com/wiki/doc/api/jsapi.php?chapter=9_3 </returns>
        public static dynamic CloseOrder(string appid, string mch_id, string transaction_id, string out_trade_no, string nonce_str, string partnerKey)
        {
            ...
            var _sign = Sign(stringADict, partnerKey);//生成签名字符串
            var _postdata = CommonHelper.GeneralPostdata(stringADict, _sign);
            var _url = ApiUrlHelper.MPApiUrl.CloseOrder;
            return PostJsonAsync(_url, _postdata);
        }
        ...
        /// <param name="refund_account">(String(30) 仅针对老资金流商户使用REFUND_SOURCE_UNSETTLED_FUNDS---未结算资金退款（默认使用未结算资金退款）REFUND_SOURCE_RECHARGE_FUNDS---可用余额退款 ) </param>
        /// <param name="partnerKey">API密钥</param>
        /// <returns> 参见：https://pay.weixin.qq.com/wiki/doc/api/jsapi.php?chapter=9_4 </returns>
        public static dynamic RefundOrder(string appid, string mch_id, string device_info, string nonce_str,
                                    string transaction_id, string out_trade_no, string out_refund_no,
                                    int total_fee, int refund_fee, string refund_fee_type, string op_user_id, string refund_account,
                                    string partnerKey)
        {
            var stringADict = new Dictionary<string, string>();
            stringADict.Add("appid", appid);
            ...
            stringADict.Add("refund_account", refund_account);

            var _sign = Sign(stringADict, partnerKey);//生成签名字符串
            var _postdata = CommonHelper.GeneralPostdata(stringADict, _sign);
            var _url = ApiUrlHelper.MPApiUrl.RefundOrder;
            return PostXmlAsync(_url, _postdata);
        }
        ...
        /// <param name="partnerKey">API密钥</param>
        /// <returns> 参见：https://pay.weixin.qq.com/wiki/doc/api/jsapi.php?chapter=9_5 </returns>
                            ...
                            string partnerKey)
        {
            ...
            var _sign = Sign(stringADict, partnerKey);//生成签名字符串
            var _postdata = CommonHelper.GeneralPostdata(stringADict, _sign);
            var _url = ApiUrlHelper.MPApiUrl.RefundQuery;
            return PostXmlAsync(_url, _postdata);
        ...
        /// <param name="tar_type">String(8) 非必传参数，固定值：GZIP，返回格式为.gzip的压缩包账单。不传则默认为数据流形式。 </param>
        /// <param name="partnerKey">API密钥</param>
        /// <returns> 
        /// 成功时，数据以文本表格的方式返回，第一行为表头，后面各行为对应的字段内容，字段内容跟查询订单或退款结果一致，具体字段说明可查阅相应接口。 
        /// 参见：https://pay.weixin.qq.com/wiki/doc/api/jsapi.php?chapter=9_6 </returns>
                                     ...
                                     string partnerKey)
        {
            ...
            stringADict.Add("tar_type", tar_type);

            var _sign = Sign(stringADict, partnerKey);//生成签名字符串
            var _postdata = CommonHelper.GeneralPostdata(stringADict, _sign);
            var _url = ApiUrlHelper.MPApiUrl.Downloadbill;
            return PostXmlAsync(_url, _postdata);
        }
        ...
        /// </param>
        /// <param name="execute_time">Int  接口耗时   接口耗时情况，单位为毫秒 </param>
        ...
        public static dynamic Report(string appid, string mch_id, string device_info, string nonce_str,
                             string interface_url, string execute_time,
                             ...
                             string partnerKey)
        {
            ...
            stringADict.Add("execute_time", execute_time);
        ...
        public static string Sign(IDictionary<string, string> stringADict, string partnerKey)
        {
            ...
            var sign = CommonHelper.MD5(stringSignTemp, "UTF-8").ToUpper();//对stringSignTemp进行MD5运算，再将得到的字符串所有字符转换为大写，得到sign值signValue。 
            return sign;
        }
    }
}
