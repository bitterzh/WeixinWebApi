...
namespace Totyu.WeixinSDK.Apis.Pay
{
    internal static class WxPayAPI2
    {
        ...
        public static WxPayData Micropay(WxPayData inputObj, int timeOut = 10)
        {
            ...
            inputObj.SetValue("spbill_create_ip", GlobalContext.SpbillCreateIp);//终端ip
            ...
            ReportCostTime(_url, timeCost, result);//测速上报
            return result;
        }
        ...
        public static WxPayData ShortUrl(WxPayData inputObj)
        {
            ...
            var start = DateTime.Now;//请求开始时间
            var _url = ApiUrlHelper.MPApiUrl.ShortUrl;
            string response = HttpClientHelper.HttpClientPostStringAsync(_url, xml);
            var end = DateTime.Now;
            int timeCost = (int)((end - start).TotalMilliseconds);
            WxPayData result = new WxPayData();
        ...
        public static WxPayData UnifiedOrder(WxPayData inputObj)
        {
            ...
            if (!inputObj.IsSet("notify_url"))
                inputObj.SetValue("notify_url", GlobalContext.PayNotifyUrl);//异步通知url
            inputObj.SetValue("appid", GlobalContext.AppID);//公众账号ID
            inputObj.SetValue("mch_id", GlobalContext.MCHId);//商户号
            inputObj.SetValue("spbill_create_ip", GlobalContext.SpbillCreateIp);//终端ip	  	
            inputObj.SetValue("sign", inputObj.MakeSign());//签名
        ...
        {
            //如果不需要进行上报
            if (GlobalContext.Report_Levenl == 0)
                return;

            //如果仅失败上报
            if (GlobalContext.Report_Levenl == 1 && inputObj.IsSet("return_code") && inputObj.GetValue("return_code").ToString() == "SUCCESS" &&
