...
namespace Totyu.WeixinWebApi.Controllers
{
    public class WeixinPayController : Controller
    {
        ...
        public JsonResult Index()
        {
            ...
            var spbill_create_ip = (trade_type == "APP" || trade_type == "NATIVE") ? Request.UserHostName : WeixinConfig.SpbillCreateIp;
            ...
            var content = WxPayAPI.UnifiedOrder(
                          ...
                          goods_tag, notify_url, trade_type, product_id, "no_credit", openid, partnerKey);
        ...
        public ActionResult Refund()
        {
            ...
            Log.Instence.LogWriteLine(requestContent);//记录请求关键信息到日志中去
            bool isVerify = false;
        ...
        {
            dynamic _result = WxMicroPayAPI.Submit(WeixinConfig.AppID, WeixinConfig.mch_id, WeixinConfig.DeviceInfo, CommonHelper.CreateNonceStr(), body, "", "test",
                                                    CommonHelper.CreateOutTradeNo(), Convert.ToInt32(fee), "CNY", "8.8.8.8", DateTime.Now.ToString("yyyyMMddHHmmss"), DateTime.Now.AddMinutes(10).ToString("yyyyMMddHHmmss"),
                                                    "jjj", "NATIVE", auth_code, WeixinConfig.PartnerKey);
        ...
        {
            dynamic _result = WxPayAPI.RefundQuery(WeixinConfig.AppID, WeixinConfig.mch_id, WeixinConfig.DeviceInfo, CommonHelper.CreateNonceStr(),
                transactionId, outTradeNo, outRefoundNo, refundId, WeixinConfig.PartnerKey);
        ...
        [HttpPost]
        public void ReportOrder(string billDate, string billType)
        {
            //dynamic _result = WxPayAPI.Report(WeixinConfig.AppID, WeixinConfig.mch_id, WeixinConfig.DeviceInfo, CommonHelper.CreateNonceStr(),
            //    billDate, billType, "GZIP", WeixinConfig.PartnerKey);
            Response.Write("<textarea rows='3' cols='2' style='color:#00CD00;font-size:20px;width:98%;'>not user function</input>");
        }
        [HttpGet]
